...
namespace Spatie\ShikiPhp\Tests\testfiles;
...
class CustomHtmlRenderer
{
    ...
    const FONT_STYLE_TO_CSS = [
        self::STYLE_ITALIC => 'font-style: var(--shiki-font-style, italic)',
        self::STYLE_BOLD => 'font-weight: var(--shiki-font-weight, bold)',
        self::STYLE_UNDERLINE => 'text-decoration: var(--shiki-font-underline, underline)'
    ];
    ...
    public function __invoke(array $tokens, array $options = [])
    {
        ...
        $html = sprintf('<pre class="%s" style="background-color: var(--shiki-color-background, %s)">', $className, $tokens['theme']['bg'] ?? '#000');
        ...
        foreach($tokens['tokens'] as $index => $line) {
            ...
            foreach($line as $token) {
                $cssDeclarations = sprintf('color: %s', $token['color'] ?? ('var(--shiki-color-text, '.$tokens['theme']['fg']??'#000'.')'));
